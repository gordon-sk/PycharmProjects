#
# Program 6.4 Solving the wave equation which does not split the initial pulse
# Derived from the 6.3 animation, with the addition of the noSplit choice which can take values of -1, 0, or 1
# The noSplit = -1 means that the non-split wave moves initially to the left end of the string
# The noSplit = +1 means that the non-split wave moves initially to the right end of the string
# The noSplit = 0 (default) means that the wave splits as in the previous versions V1 to V3
...
#
# 3) Output (see Figure 6.2, page 160)
...
parser.add_argument('--omega', default=15, type=float, help="Omega value for oscillating end, default 20pi/s")
...
parser.add_argument('--movingEnd', action='store_false', help="Allow the left end of the string to oscillate; default True")
parser.add_argument('--verbose', action='store_true', help="Give extra printout; default False")
parser.add_argument('--noSplit', default=1, type=int, choices=[-1, 0, 1], help="No split choice; default 0")
...
noSplit = args.noSplit      # option for not splitting the initial pulse
movingEnd = args.movingEnd  # option for the left end of the string to oscillate, creating a wave
omega = args.omega * mp.pi
...
print "\n  Program to solve the wave equation for a string and to produce an animation"
...
if(noSplit != 0):
    print "  Initial pulse will not split"
...
def initializeWave(yxt1, yxt2, xMin, xStep, kFactor, initialX0):
    ...
    if movingEnd:
        ...
        for i in range(nStepsX):
            yxt2[i] = gaussAmplitude*mp.exp(-kFactor*(xValue - initialX0)*(xValue - initialX0))

            if(noSplit == 0):
                yxt1[i] = yxt2[i]        # previous time step had the same wave form at the same x0
            else:
                previousX0 = initialX0 - noSplit*tStep*cSpeed  # previous time step at the wave form with a different x0
                yxt1[i] = gaussAmplitude*mp.exp(-kFactor*(xValue - previousX0)*(xValue - previousX0))
...
def initializeEnds(yxt1, yxt2, yxt3):
    ...
    if movingEnd:
        # y(0,t) = Asin(omega*t)
        yxt1[0] = gaussAmplitude * mp.exp(-kFactor*(initialX0 - xStep)*(initialX0 - xStep))
        yxt2[0] = gaussAmplitude * mp.exp(-kFactor*(initialX0)*(initialX0))
        yxt3[0] = gaussAmplitude * mp.exp(-kFactor*(initialX0 + xStep)*(initialX0 + xStep))
...
def animateFixedString(nStepsX):
    ...
    plt.xlim(0.0, xMax)
    plt.ylim(-4, 4)
    baselinePlotX = [0.0, xMax]
    baselinePlotY = [0.0, 0.0]
    plt.plot(baselinePlotX, baselinePlotY, 'k')

    if(freeEnd):
        plt.title('Wave Propagation in a String with One Free Endpoint')
    else:
        plt.title('Wave Propagation in a String with Fixed Endpoints')
...
#
    if(doMovie):
        ...
        ani.save('waveOnAFixedString.mp4', writer = FFwriter)
...
for n in range(nStepsT):
    ...
    if movingEnd:
        print currentTime
        yxt1[0] = gaussAmplitude * mp.sin(omega * (currentTime - tStep))
        yxt2[0] = gaussAmplitude * mp.sin(omega * currentTime)
        yxt3[0] = gaussAmplitude * mp.sin(omega * (currentTime + tStep))
    for i in range(nStepsX):
...
# Fourth problem is from chapter 6
# fixed string, right end is fixed, not free to move...
# left end is    y(0, t) = Asin(omega*t)
# So at all times you already know that the x(0) pos is bc it is given by that equation
# so at t=0 string is straight...
# there's a little dip at the previous time step...
# at the next time step its a little higher..........
#